...
class TestAirPortInfoEndpoint(APITestCase):
    ...
    def test_put_airport(self):
        ...
        res_fail_put = self.client.put(reverse(self.airport_url, kwargs={"pk": self.airport_id}))
        self.assertEqual(res.status_code, status.HTTP_200_OK)
...
class TestFlightEndpoint(APITestCase):
    def setUp(self):
        ...
        self.sarch_arrival_url = "sarch-arrival"
        self.aircraft_info = {
            "serial_number": "yuoalr-rlaovun-niaps-anolecrab",
            "manufacturer": "Scorpion"
        }
    ...
    def test_put_flight(self):
        ...
        res = self.client.put(reverse(self.edit_flight_url, kwargs={"pk": self.flight_id}), flight_info )
    ...
    def test_patch_flight(self):
        ...
        res = self.client.patch(reverse(self.edit_flight_url, kwargs={"pk": self.flight_id}), flight_info )
    ...
    def test_search_by_departure(self):
        res = self.client.get(reverse(self.search_departure_url, kwargs={"icao":"883GG"}))
        self.assertEqual(res.status_code, status.HTTP_200_OK)
        self.assertEqual(res.status_code, status.HTTP_200_OK)
